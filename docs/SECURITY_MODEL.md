...
- **Real accrual location (per tier, not the originally-designed Midnight PSMs — see 2):** Cardano Launch — `bonding_curve_tier_b.ak` (Cardano, both public-phase and DarkVeil-claim fees). Midnight Launch — `bonding_curve.compact`'s inline ledger fields.
