...
- **Language:** Aiken
- **Indexer:** Blockfrost API (primary, real and live). Failover is **partial**, not absent (corrected 2026-08-02 — this line previously claimed "no fallback client exists yet," which was wrong): a real `koios-client.php` exists in the plugin and is used for chain-tip health, `account_assets`, and `account_info` (the Settings page surfaces it as "Not set — Koios fallback active", and there is a `np/v1/health/koios` route). **Corrected 2026-09-17 — the TypeScript side now has a failover router, and the previous "no failover of any kind" wording is out of date.** `integration/chain-provider-router.ts` and `integration/chain-backends.ts` implement Blockfrost→Koios failover behind a circuit breaker. Read the scope precisely rather than the headline: it routes exactly **four read methods** — `getLatestBlock`, `getAddressInfo`, `getAddressUtxosAll`, `getAddressTransactionsAll` — and **transaction submission is not among them**. Only three files reference the router today (itself, its backends module, and its test), so no submitter is wired through it yet. **Maestro is still unimplemented**: the documented order is Blockfrost → Maestro → Koios, and Maestro needs an API key nobody has provisioned. Wiring the submitters through the router, and adding a second Blockfrost-compatible backend, is the remaining work.
- **Transaction building:** Anvil API for standard transactions; **`@lucid-evolution/lucid`** (Anastasia Labs) for custom-redeemer Plutus script spends, which Anvil cannot do. ⚠️ **Supply-chain watch item (2026-08-02):** the original Lucid Evolution core developers have left to form No Witness Labs and are building a separate, ground-up successor — **Evolution SDK** (`@evolution-sdk/evolution`, IntersectMBO-incubated, pure TypeScript, no WASM/CML). Our package remains actively maintained by Anastasia Labs — 0.6.0 (2026-07-16), 0.6.1 (2026-08-09), 0.6.2 (2026-08-13), no deprecation notice — so **no migration now**. Both re-evaluation triggers were re-checked against the registry on 2026-08-21 and **neither has fired**: Evolution SDK is still 0.5.12, and three releases in four weeks is not a stalled cadence. Institutional momentum still sits with the successor, and migration would be a near-total call-site rewrite across **83 files** — 33 in `integration/`, 34 tests, 11 CLI entry points, 5 in the widget. Re-evaluate when Evolution SDK 2.0 ships or if Anastasia's release cadence stalls. **We run `^0.6.2`, resolving 0.6.2** (corrected 2026-09-17 — this line said `^0.5.5`/0.5.6 long after the upgrade landed on 2026-09-01). The step across the minor was deliberate, as a caret on a `0.x` version cannot cross one on its own. It was taken before the security audit so the audited code is the shipped code. The two things to keep re-checking against a Preprod lifecycle run are what 0.6.0 changed and what our submitters lean on hardest: wallet UTxO-override semantics, and slot configuration having moved per-instance. **Do NOT install `@evolution-sdk/lucid`** — despite the high version number (2.0.1) it has only ever had two releases, both in July 2025, and a No Witness Labs fork README actively instructs people to install it.
...
> **Resolution (2026-07-13):** step 2's registration freeze now enforces a real minimum — `registered_count` must reach `MIN_DV_PARTICIPANTS` (15) before buying can open, or the governor must cancel DarkVeil instead (existing failure path, fully refundable). This closes two griefing vectors a percentage-only check couldn't — full exploit detail in `local/SECURITY_AUDIT.md`, not published here by policy. Enforced on-chain — `openBuying` in both `eligibility_gate.compact` (Cardano Launch) and `bonding_curve.compact` (Midnight Launch) refuses the Registration → Buying transition below the floor, a deploy-time `minDvParticipants` ledger field set from this constant. A launch that never reaches the floor never opens, and `expireDarkVeil` returns every bond in full on the clock. 6 new tests (3 per contract) pass; 193/193 total Compact tests pass (was 187).
